...
public class MemberView {
	...
	public void statusJoin(int row) {
		System.out.println("══════════════════════");
		if (row > 0) { // row = 영향을 받은 행이 있는지 없는지 즉 회원가입을 하면 db에 sql구문에 영향을 받은 행이 하나 더 생겨서 회원가입성공 출력
						// 실패시 영향을 받은 행이없으므로 출력x
	...
	public void showStats(ArrayList<MemberVO> list) {
		...
		for (MemberVO mvo : list) {
			System.out.printf("%-10s | %-7d | %-10.2f | %-7.2f%%%n", mvo.getId(), mvo.getTotalScore(), mvo.getAvgTry(),
					mvo.getWinRate());
		}
	}
