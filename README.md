...
| **클래스 명** | UserDAOImplement |
| --- | --- |
| **이슈 사항** | Too many connections 오류 출력됨 |
| **원인** | UserDAOImplement에서 오류가 발생하였고 connection이 많다 라고 출력되니 DB 연결 시도가 계속 누적되고 있다고 판단됨 |

## ⁉해결 단계

- **시도1. Database 서버 재시작**
    
    계속해서 같은 오류가 발생하여 Database 서버 재시작을 요청드렸다. 이후 문제가 해결된 것 같았지만 또다시 같은 오류가 발생함
    
- **시도2. close() 호출**
    
    코드를 다시 확인했을 때 Connection 객체를 close() 하지 않은 것을 추가하여 코드를 수정하였다
    

## ❗해결 완료

| **해결 방법** | DAO 클래스의 모든 메서드에서 close()를 호출하는 것으로 코드 수정 |
| --- | --- |
